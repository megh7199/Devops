...
const EventList = () => {
  ...
  useEffect(() => {
        ...
        setLoading(true);
        console.log('check1')
				const { data } = await axios.get(`https://app.ticketmaster.com/discovery/v2/events?countryCode=US&apikey=${apiKey}&page=`+pagenum);
        ...
        setNextPage(false);
			...
			// 	const { data } = await axios.get(`https://app.ticketmaster.com/discovery/v2/events?countryCode=US&apikey=${apiKey}&page=`+next);
		...
		() => {
			...
			async function fetchData() {
				try {
					...
					const { data } = await axios.get(`https://app.ticketmaster.com/discovery/v2/events?countryCode=US&apikey=${apiKey}&keyword=${searchTerm}`);
					console.log(data)
              ...
              <CardContent>
                ...
                <Typography variant='body2' color='textSecondary' component='p'>
                  {show.classifications && show.classifications[0].genre && show.classifications[0].genre.name? show.classifications[0].genre.name : <dd>N/A</dd>}
									...
									{show.promoter && show.promoter.description ? <dd>| {show.promoter.description.replace(regex, '').substring(0, 139) + '...' }</dd>: 'No Summary'}
									<span>More Info</span>
                </Typography>
              </CardContent>
            </Link>
    ...
    return (
      <div>
        ...
        <br />
				<br />
				<div>
					{pagenum!=0?<Link to={`/events/page/${parseInt(pagenum) - 1}`}> Previous page       </Link>:<></>}
					{nextPage?<Link to={`/events/page/${parseInt(pagenum )+ 1}`}>     Next page </Link>:<></>}
				</div>
				<br></br>
				<br></br>
        <Grid
